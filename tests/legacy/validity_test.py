...
def read_bag(bag_path):
    ...
    rgb_topic_id, rgb_type = topic_map[
        RGB_TOPIC
    ]

    depth_topic_id, depth_type = topic_map[
        DEPTH_TOPIC
    ]
    ...
    if camera_info_topic_id is not None:
        ...
        print(
            f"CameraInfo ID:  {camera_info_topic_id}"
        )

        print(
            f"CameraInfo type:{camera_info_type}"
        )

    else:

        print(
            "CameraInfo:     NOT FOUND"
        )
...
def configure_camera_info(camera_info):
    ...
    if camera_info is not None:
        ...
        print(
            "Using recorded CameraInfo."
        )

        print(
            f"Recorded fx = {recorded_fx}"
        )

        print(
            f"Recorded fy = {recorded_fy}"
        )

        print(
            f"Recorded cx = {recorded_cx}"
        )

        print(
            f"Recorded cy = {recorded_cy}"
        )
    ...
    print()
    print(
        f"Active fx = {active_fx}"
    )

    print(
        f"Active fy = {active_fy}"
    )

    print(
        f"Active cx = {active_cx}"
    )

    print(
        f"Active cy = {active_cy}"
    )
...
def match_features(
    ...
    if des1 is None or des2 is None:
        ...
        return (
            kp1 if kp1 is not None else [],
            kp2 if kp2 is not None else [],
            [],
            0
        )
...
def process_odometry(pairs):
    ...
    depth0_raw = image_message_to_numpy(
        depth_msg0
    )
    ...
    print(
        f"  RGB:   {rgb_msg0.encoding}"
    )

    print(
        f"  Depth: {depth_msg0.encoding}"
    )
    ...
    if len(valid_depth0) > 0:
        ...
        print(
            f"  Min valid depth:  "
            f"{np.min(valid_depth0):.4f} m"
        )

        print(
            f"  Max valid depth:  "
            f"{np.max(valid_depth0):.4f} m"
        )

        print(
            f"  Mean valid depth: "
            f"{np.mean(valid_depth0):.4f} m"
        )
    ...
    print(
        f"  Valid pixels: "
        f"{np.sum(valid0)} / {depth0.size}"
    )

    print(
        f"  Valid percentage: "
        f"{100.0 * np.sum(valid0) / depth0.size:.2f}%"
    )

    prev_gray = prepare_rgb(
        rgb0
    )
    ...
    for i in range(
        ...
        curr_depth_raw = image_message_to_numpy(
            curr_depth_msg
        )
        ...
        # ----------------------------------------------------
        # RGB-depth timestamp difference
        # ----------------------------------------------------
        ...
        if result is None:
            ...
            mean_residual = result[
                "mean_residual"
            ]

            median_residual = result[
                "median_residual"
            ]
            ...
            if accepted:
                ...
                #       R * P_previous + t
                #
                # Therefore the camera motion in world
                # coordinates is:
                #
                # R_world_current =
                #       R_world_previous * R^T
                #
                # t_world_current =
                #       t_world_previous
                #       - R_world_current * t
                #
                # This assumes the world frame is the
        ...
        # Save trajectory
        #
        # If rejected/failed, pose stays at last trusted pose.
        # ----------------------------------------------------
        ...
        #
        # We continue using the current frame as the next
        # tracking reference, even if the current pose was
        # rejected.
        # ----------------------------------------------------
...
def print_summary(
    ...
    if diagnostics:
        ...
        if accepted_translations:
            ...
            print(
                f"  Average accepted motion: "
                f"{np.mean(accepted_translations):.5f} m"
            )

            print(
                f"  Maximum accepted motion: "
                f"{np.max(accepted_translations):.5f} m"
            )
